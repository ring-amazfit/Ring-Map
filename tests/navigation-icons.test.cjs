...
for (const name of names) {
  ...
  assert(info.width === 128 && info.height === 128, `icon must match its 128px widget: ${name}`)
...
for (const target of ['balance', 'gtr4', 'cheetahpro', 'active2', 'trex3', 'trex3-pro']) {
  for (const name of names) {
    ...
    assert(info.width === 128 && info.height === 128 && info.colorType === 6, `packaged ${target} icon wrong: ${name}`)
  }
}
